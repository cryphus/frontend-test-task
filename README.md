...
- Vue 3.5, `<script setup>`, Vite, vue-router
- Шрифт Inter подключён локально (`@fontsource/inter`)

## Правила компонентов

- Каждый UI-элемент — отдельный компонент в отдельном файле; нативный `<button>` есть только в `avanti_button.vue` и `avanti_icon_button.vue`.
- Имена файлов: `avanti_<назначение>.vue` / `avanti_<назначение>.js`.
- Максимальный размер файла — 230 строк.
- Нет `style="..."` и логики в шаблонах: условия, строки и классы вычисляются в `computed`, обработчики — именованные функции.
- Стили — только `<style scoped>`, общие токены — CSS-переменные в `resources/css/app.css`.
